...
class Correlate:
  ...
  def correlate(classElem, ignores): #TODO ignores
    ...
    res_mkConstMethod, _remaining, _features = Correlate.filter_mkConstMethod(methods, ignore=lambda x: not x in boolisprop_remaining and not x in prop_remaining)
    features += _features
    remaining += [ _remaining ]
